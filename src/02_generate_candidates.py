...
def deduplicate(df: pd.DataFrame, embedder: SentenceTransformer, threshold: float, model_name_str: str) -> Tuple[
    ...
    if df.empty or 'question_text' not in df.columns:
        ...
        return df, []  # Return the empty df and an empty list of scores
    ...
    try:
        ...
        # to get a flat list of unique pairwise scores.
        # This requires numpy, which is a core dependency of pandas.
        import numpy as np
    ...
    # --- The original deduplication logic remains the same ---
    indices_to_keep = []
    print(f"Deduplication: Filtering questions with threshold {threshold:.2f}...")
    for i, emb_i in enumerate(embeddings):
        is_duplicate = False
        # Compare against all questions already kept
    ...
    # Return both the DataFrame and the list of scores
...
# ------------------ STEP 6: RAGAS Filter ------------------------------------
def ragas_filter(df: pd.DataFrame, pages_data: List[Dict], threshold: float) -> Tuple[
    pd.DataFrame, List[float], List[float]]:
    """
    Filters DataFrame based on Ragas scores, adds the scores to the DataFrame,
    and returns lists of the collected scores.
    """
    if 'ragas_eval' not in globals() or 'faithfulness' not in globals() or 'answer_correctness' not in globals() or 'Dataset' not in globals():
        logging.warning("Ragas/Datasets components not available. Skipping RAGAS filter.")
        return df, [], []
    if df.empty:
        logging.info("Ragas Filter: Skipping, DataFrame is empty.")
        return df, [], []
    if not pages_data:
        logging.warning("Ragas Filter: Missing pages_data. Skipping filter.")
        return df, [], []
    ...
    page_lookup = {page['page_num']: page.get('markdown_content', '') for page in pages_data if
                   isinstance(page.get('page_num'), int) and isinstance(page.get('markdown_content'), str)}
    if not page_lookup:
        logging.warning("Ragas Filter: Failed page lookup creation. Skipping filter.")
        return df, [], []

    ragas_data_list = []
    original_indices = []
    # Prepare data only for rows that need evaluation
    for index, row in df[df['category'] != UNANSWERABLE_CATEGORY_NAME].iterrows():
    ...
    if not ragas_data_list:
        logging.warning("Ragas Filter: No valid data to evaluate. Keeping all rows.")
        return df, [], []

    print(f"Ragas Filter: Prepared {len(ragas_data_list)} rows for evaluation.")
    ...
    try:
        ...
        logging.warning("Ragas Filter: Skipping RAGAS filtering due to error.")
        return df, [], []
    ...
    # Create a new DataFrame with scores and the original indices to merge on
    ...
    # Join the scores with the main DataFrame
    df = df.join(scores_to_merge)

    # Fill scores for rows that were not evaluated (e.g., Unanswerable) with -1.0
    ...
    passed_ragas_mask = (df['ragas_faithfulness'] >= threshold) & (df['ragas_correctness'] >= threshold)

    df_filtered = df[unanswerable_mask | passed_ragas_mask].reset_index(drop=True)
    # --- END of updated logic ---

    n_rows_after = len(df_filtered)
    n_removed = n_rows_before - n_rows_after
    print(f"Ragas Filter: Kept {n_rows_after} / {n_rows_before} rows (Removed {n_removed}).")
    print(f" Shape after Ragas: {df_filtered.shape}")
    print("\n--- Step 6 Complete ---")

    return df_filtered, faithfulness_scores_list, correctness_scores_list
...
def judge_filter(df: pd.DataFrame, pages_data: List[Dict], judge_model_name: str, threshold: int) -> Tuple[
    pd.DataFrame, List[int]]:
    """
    Filters DataFrame based on scores from an LLM judge, adds the scores to the
    DataFrame, and returns a list of the collected scores.
    """
    openai_available = 'openai' in sys.modules and hasattr(sys.modules['openai'], 'chat')
    if not openai_available:
        logging.warning("OpenAI client not available/configured. Skipping Judge filter.")
        return df, []
    if df.empty:
        logging.info("Judge Filter: Skipping, DataFrame is empty.")
        return df, []
    if not pages_data:
        logging.warning("Judge Filter: Missing pages_data. Skipping filter.")
        return df, []
    ...
    page_lookup = {page['page_num']: page.get('markdown_content', '') for page in pages_data if
                   isinstance(page.get('page_num'), int) and isinstance(page.get('markdown_content'), str)}
    if not page_lookup:
        logging.warning("Judge Filter: Failed lookup creation. Skipping filter.")
        return df, []
    ...
    for i, index in enumerate(indices_to_judge):
        ...
        if not isinstance(page_content, str) or not page_content:
            ...
            continue
        try:
            score = gpt_judge_score(question, answer_snippet, page_content, judge_model_name)
            scores[index] = score
    ...
    # Fill scores for rows that were not judged (e.g., Unanswerable) with -1
    df['judge_score'] = df['judge_score'].fillna(-1)
    # Convert the column to integer type for cleanliness
    df['judge_score'] = df['judge_score'].astype(int)

    # Get a list of the actual scores given by the judge for stats
    judge_scores_list = list(scores.values())
    # --- END of new code ---
    ...
    print(f" Shape after Judge: {df_filtered.shape}")
    print("\n--- Step 7 Complete ---")

    # Return both the filtered DataFrame (with the new column) and the list of scores
    return df_filtered, judge_scores_list
...
def export_audit_slice(df: pd.DataFrame, output_base_path: Path, audit_fraction: float, random_seed: int = 42):
    ...
    if df.empty:
        ...
        # Return default values that won't cause an error in the main block
        return 0, 0

    # --- Define New Procedural Sentinel ---
    ...
    # --- Save Candidates FIRST (in JSONL format) ---
    ...
    try:
        if 'corrected_steps' not in df.columns: df['corrected_steps'] = None
        if 'procedural_comments' not in df.columns: df['procedural_comments'] = None
    ...
    # --- Stage 1 Audit: General Quality (Non-Procedural, Non-Unanswerable) ---
    print("\nAudit Export (Stage 1): Preparing General Quality audit files...")
    eligible_categories = [cat for cat in VALID_CATEGORIES if cat not in [PROCEDURAL_CATEGORY_NAME, UNANSWERABLE_CATEGORY_NAME]]
    df_audit_pool_s1 = df[df['category'].isin(eligible_categories)]
    population_size = len(df_audit_pool_s1)

    # --- REVERTED: Calculate sample size based on audit_fraction ---
    if population_size > 0:
        num_to_sample = math.ceil(population_size * audit_fraction)
    else:
        num_to_sample = 0
    # Ensure we never try to sample more than what's available
    ...
    for rater in ["A", "B"]:
        ...
        for label_col in label_cols_s1: audit_df[label_col] = None
    ...
    # --- Stage 2 Audit: Procedural Quality ---
    ...
    if num_procedural > 0:
        ...
        for rater in ["A", "B"]:
            ...
            for label_col in label_cols_s2: audit_df[label_col] = None
    ...
    # --- Stage 3 Export: Procedural Correction File (Original logic retained) ---
...
if __name__ == "__main__":
    ...
    # Define paths for key output files
    ...
    for k, v in config.items():
        try:
            ...
            serializable_config[k] = str(v)  # Convert non-serializable to string
        else:
    ...
    try:  # Wrap pipeline steps
        ...
        # --- REFACTORED: Always build prompt and count tokens for consistent stats ---
        ...
        try:
            ...
            count_response = gemini_model.count_tokens(full_prompt)
            ...
            run_stats['prompt_token_count'] = -1  # Use -1 to indicate an error
        # --- End of refactored block ---

        loaded_raw_rows = []
        # Now, conditionally generate new rows or load existing ones
        if raw_output_file_path.is_file():
            ...
            generation_temp = 0.8
            raw_generated_rows = over_generate(full_prompt, OVERGEN_FACTOR, gemini_model, generation_temp)
        ...
        # Step 3b: Add Parsed Steps via LLM
        df = add_parsed_steps_llm(df, STEP_PARSING_MODEL)
        ...
        rows_before = len(df)
        if not df.empty:
            # Capture both the dataframe and the similarity scores
            df, similarity_scores = deduplicate(df, embedding_model, DUP_THRESHOLD, EMBED_MODEL)
            # Store the scores in the run stats
            run_stats['deduplication_similarity_scores'] = similarity_scores
        else:
            run_stats['deduplication_similarity_scores'] = []  # Store empty list if no dedupe was run
        ...
        rows_before = len(df)
        if not df.empty:
            # Capture the DataFrame and the two lists of RAGAS scores
            df, faithfulness_scores, correctness_scores = ragas_filter(df, loaded_pages, RAGAS_THRESHOLD)
            # Store the lists of scores in the run stats
        ...
        run_stats['rows_removed_ragas'] = rows_before - len(df)
        if df.empty: logging.warning("DataFrame empty after Ragas filtering.")

        # Step 7: Judge Filter
        rows_before = len(df)
        if not df.empty:
            # Capture the DataFrame with the new 'judge_score' column and the list of scores
            df, judge_scores = judge_filter(df, loaded_pages, JUDGE_MODEL, JUDGE_THRESHOLD)
            # Store the list of scores in the run stats
            run_stats['judge_scores'] = judge_scores
        else:
            run_stats['judge_scores'] = []  # Store empty list if no judging was run

        run_stats['rows_after_judge'] = len(df)
        run_stats['rows_removed_judge'] = rows_before - len(df)
        ...
        if not df.empty:
            ...
            if not df_unans.empty:
                ...
                verified_idx = verify_unanswerable(df_unans, full_ctx, JUDGE_MODEL)
        ...
        if len(df) == 0: raise ValueError("Quota selection resulted in zero rows. Cannot proceed.")

        # Step 9: Save Candidates & Export Audit Slice
        if df.empty or len(df) < 1: raise ValueError(
            f"Quota selection failed or resulted in empty DataFrame. Cannot export.")

        # Call export_audit_slice and capture the returned sample sizes
        general_audit_size, procedural_audit_size = export_audit_slice(df, output_base_path, AUDIT_FRACTION)

        # Store the correct audit sample sizes in the run stats
        # The number of rows in the file is the number of samples + 1 sentinel
        run_stats['audit_sample_size_stage1'] = general_audit_size + NUM_SENTINELS
        # Add 1 for the sentinel only if procedural questions were actually exported
        ...
        # Dynamically create the new audit file names for the printout
